  ...
  useEffect(() => {
    const fetchWeather = async () => {
      if (userLocation.latitude && userLocation.longitude) {
        try {
const data = await getWeatherForecast(userLocation.latitude, userLocation.longitude);
          setForecast(data);
        } catch (err) {
    ...
    <div className={`${isDarkMode ? 'dark' : ''} header`}>
      ...
      {isConnected && memberConnected && (
        <div className="weather-info">
          ...
          {forecast && (
            <div>
              {forecast.list.slice(0, 1).map((day, index) => (
                <div key={index} className="weather-info_content">
                  <p className="weather-info_city">{forecast.city.name || 'Paris'}</p>
                  <p className="weather-info_temp">{day.main.temp}°C</p>
                  <img
